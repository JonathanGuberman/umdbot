...
def make_deck_embed(deck):
    ...
    for hero in deck["heroes"]:
        if len(deck["heroes"]) > 1:
            fields.append(
                interactions.EmbedField(
                    name="Hero Name", value=hero["name"], inline=False
                )
            )
        ...
        if hero["attack_type"]:
            fields.append(
                interactions.EmbedField(
                    name="Attack Type", value=hero["attack_type"].title(), inline=True
                )
            )

        if hero["hp"]:
            fields.append(
                interactions.EmbedField(name="HP", value=hero["hp"], inline=True)
            )

        if hero["quantity"] > 1:
            fields.append(
                interactions.EmbedField(
                    name="Quantity", value=hero["quantity"], inline=True
                )
            )

    if deck["movement"]:
        fields.append(
            interactions.EmbedField(name="Move", value=deck["movement"], inline=False)
        )
    ...
    for sidekick in deck["sidekicks"]:
        if sidekick["name"]:
            fields.append(
                interactions.EmbedField(
                    name="Sidekick Name", value=sidekick["name"], inline=False
                )
            )
        ...
        if sidekick["attack_type"]:
            fields.append(
                interactions.EmbedField(
                    name="Sidekick Attack Type",
                    value=sidekick["attack_type"].title(),
                    inline=True,
                )
            )
        ...
        if sidekick["hp"]:
            fields.append(
                interactions.EmbedField(
                    name="Sidekick HP", value=sidekick["hp"], inline=True
                )
            )
        ...
        if sidekick["quantity"] and sidekick["quantity"] > 1:
            fields.append(
                interactions.EmbedField(
                    name="Sidekick Quantity", value=sidekick["quantity"], inline=True
                )
            )
    ...
    if deck["special"]:
        fields.append(
            interactions.EmbedField(
                name="Special Ability", value=deck["special"], inline=False
            )
        )

    if deck["notes"]:
        fields.append(
            interactions.EmbedField(name="Notes", value=deck["notes"], inline=False)
        )
...
async def deck(ctx, name):
    ...
    if len(decks) == 1:
        ...
        await ctx.send(
            "Multiple matching decks found",
            components=make_deck_select(decks),
            ephemeral=True,
        )
    else:
