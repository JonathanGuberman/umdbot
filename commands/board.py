...
def make_board_embed(board):
    ...
    if board["max_players"]:
        fields.append(
            interactions.EmbedField(
                name="Max Players", value=board["max_players"], inline=True
            )
        )
    ...
    if board["spaces_count"]:
        fields.append(
            interactions.EmbedField(
                name="Space Count", value=board["spaces_count"], inline=True
            )
        )
    ...
    if board["zone_count"]:
        fields.append(
            interactions.EmbedField(
                name="Zone Count", value=board["zone_count"], inline=True
            )
        )

    if board["notes"]:
        fields.append(
            interactions.EmbedField(name="Notes", value=board["notes"], inline=False)
        )
...
async def board(ctx, name):
    ...
    if len(boards) == 1:
        ...
        await ctx.send(
            "Multiple matching boards found",
            components=make_board_select(boards),
            ephemeral=True,
        )
    else:
